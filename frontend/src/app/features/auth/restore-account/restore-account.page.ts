        ...
        @switch (state()) {
          ...
          @case ('success') {
            <div class="block">
              ...
              <p>Your account is active again. Log in to keep using the app.</p>
          ...
          @case ('error') {
            <div class="block">
              ...
              <p>If the 30-day grace period has passed, your data is already gone. Sorry.</p>
              <a mat-flat-button color="primary" routerLink="/login">Back to login</a>
            </div>
          }
          @case ('no-token') {
...
export class RestoreAccountPage implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.api.confirmAccountRestore(token).subscribe({
